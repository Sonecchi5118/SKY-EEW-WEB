...
let detectedquakes = [];
const realtimepointshistory = []; //10秒間記録する
...
function RealTimeQuake(day) {
    ...
    const operatedata = (time) => {
        ...
        for (let i = 0; i < RealTimeIntObj.length; i++) {
            ...
            const lastsecpoint = (_a = realtimepointshistory[0]) === null || _a === void 0 ? void 0 : _a.find(p => p.ind == nowpoint.ind);
            if (lastsecpoint) {
                const nearestquake = detectedquakes.sort((a, b) => {
                    return Math.sqrt(Math.pow((a.lat - np.y), 2) + Math.pow((a.lon - np.x), 2) + Math.pow(a.depth, 2)) - Math.sqrt(Math.pow((b.lat - np.y), 2) + Math.pow((b.lon - np.x), 2) + Math.pow(b.depth, 2));
                })[0]; //最も近い震源を選択
                ...
                if (nearestquake) {
                    ...
                    if (soujiindex < soujitable.length) {
                        ...
                        if (souji)
                            if ((time - nearestquake.time) > souji)
                                isinquake = true;
                    }
                }
                if (!nowpoint.detecting) {
                        ...
                        if (nowpoint.int - lastsecpoint.int > 1) { //上昇した
                            ...
                            if (aroundpoints.some(p => p.detecting)) {
                                ...
                                for (const aroundpoint of aroundpoints) {
                                    if (aroundpoint.int - ((_c = (_b = realtimepointshistory[0].find(pt => pt.ind == aroundpoint.ind)) === null || _b === void 0 ? void 0 : _b.int) !== null && _c !== void 0 ? _c : 10) > 0.5) {
                    ...
                    if (nowpoint.int - lastsecpoint.int > 0.2 || nowpoint.int >= 1.5)
                        detectedpointskeeptimelist[nowpoint.ind] = 10; //上昇したか、震度2以上なら検知延長
                    else
                        detectedpointskeeptimelist[nowpoint.ind]--; //そうでなければ検知期限を減らす
                ...
                if (nowpoint.detecting && !findregion)
                    ...
                    findregion.int = int;
                if (nearestquake)
                    nowpoint.isineew = nearestquake.matcheew;
            }
        }
        /**
         * @description 仮定震源の座標
        ...
        const calculatehypocenter = (quake, lat, lon, depth) => {
            ...
            for (let i = 0; i < RealTimeIntObj.length; i++) {
                ...
                const firstdetectpointdistance = Math.sqrt(Math.pow(haversineDistance(lat, firstdetectpoint.y, lon, firstdetectpoint.x), 2) + Math.pow(depth, 2)); //km
                ...
                if (!quake)
                    continue;
                const squaremargin = Math.pow(((pinfo.begantime - (quake.time || 0)) / 1000), 2);
        ...
        if (realtimepointshistory.length >= 10)
            realtimepointshistory.pop(); //10秒前のやつは消す
    ...
    if (isReplay) {
        ...
        for (let index = 0; index < NIEDrealTimePointLocation.length; index++) {
            //console.log(index)
            const lastsecpoint = (_a = realtimepointshistory[0]) === null || _a === void 0 ? void 0 : _a.find(p => p.ind == index);
            const IntData = ReplayIntData[index];
            if (IntData == undefined)
                RealTimeIntObj.push({ ind: index, int: -3, isfirstdetect: false, detecting: (lastsecpoint === null || lastsecpoint === void 0 ? void 0 : lastsecpoint.detecting) || false, detecttime: lastsecpoint === null || lastsecpoint === void 0 ? void 0 : lastsecpoint.detecttime, isineew: false });
            else {
                let success = false;
                for (const data of IntData) {
                    if (data.Time == runningtime) {
                        //console.log(IntData)
                        RealTimeIntObj.push({ ind: index, int: data.Intensity, isfirstdetect: false, detecting: (lastsecpoint === null || lastsecpoint === void 0 ? void 0 : lastsecpoint.detecting) || false, detecttime: lastsecpoint === null || lastsecpoint === void 0 ? void 0 : lastsecpoint.detecttime, isineew: false });
                ...
                if (!success)
                    RealTimeIntObj.push({ ind: index, int: -3, isfirstdetect: false, detecting: false, detecttime: lastsecpoint === null || lastsecpoint === void 0 ? void 0 : lastsecpoint.detecttime, isineew: false });
            }
        }
        ...
        Jimp.read(`http://www.kmoni.bosai.go.jp/data/map_img/RealTimeImg/jma_s/${now.getFullYear()}${`00${now.getMonth() + 1}`.slice(-2)}${`00${now.getDate()}`.slice(-2)}/${now.getFullYear()}${`00${now.getMonth() + 1}`.slice(-2)}${`00${now.getDate()}`.slice(-2)}${`00${now.getHours()}`.slice(-2)}${`00${now.getMinutes()}`.slice(-2)}${`00${now.getSeconds()}`.slice(-2)}.jma_s.gif`)
            ...
            for (let index = 0; index < NIEDrealTimePointLocation.length; index++) {
                ...
                if (rgba.r == 0 && rgba.b == 0 && rgba.g == 0)
                    continue;
                // 最も近い色を選択
                const intensity = IntcolorList.sort((a, b) => {
                    const distancea = Math.sqrt(Math.pow(rgba.r - a.R, 2) +
                        Math.pow(rgba.g - a.G, 2) +
                        Math.pow(rgba.b - a.B, 2));
                    const distanceb = Math.sqrt(Math.pow(rgba.r - b.R, 2) +
                        Math.pow(rgba.g - b.G, 2) +
                        Math.pow(rgba.b - b.B, 2));
                    return distancea - distanceb;
                })[0];
                if (newMaxInt < intensity.Intensity)
                    newMaxInt = intensity.Intensity;
                RealTimeIntObj.push({ ind: index, int: (_b = intensity === null || intensity === void 0 ? void 0 : intensity.Intensity) !== null && _b !== void 0 ? _b : 0, isfirstdetect: false, detecting: (lastsecpoint === null || lastsecpoint === void 0 ? void 0 : lastsecpoint.detecting) || false, detecttime: lastsecpoint === null || lastsecpoint === void 0 ? void 0 : lastsecpoint.detecttime, isineew: false });
            }
...
function EEW(data) {
    ...
    for (const area of data.WarnArea) {
        ...
        const regionname = (rawname === null || rawname === void 0 ? void 0 : rawname.endsWith('県')) || (rawname === null || rawname === void 0 ? void 0 : rawname.endsWith('府')) ? rawname.slice(0, -1) : rawname;
        if (regionname)
    ...
    const callhyponame = (delay = false) => {
        var _a;
        const memory = EEWMemory.get(data.EventID);
        if (delay ? memory === null || memory === void 0 ? void 0 : memory.hypocenter.called : hypocalled)
            return;
        const hypotihou = (_a = (delay ? memory === null || memory === void 0 ? void 0 : memory.hypocenter.name : data.Hypocenter)) === null || _a === void 0 ? void 0 : _a.endsWith('地方');
        const hypocenterInd2 = delay ? hypocenterNames.findIndex(h => h == (hypotihou ? memory === null || memory === void 0 ? void 0 : memory.hypocenter.name.slice(0, -2) : memory === null || memory === void 0 ? void 0 : memory.hypocenter.name)) : hypocenterInd;
        const isLevel2 = delay ? (memory === null || memory === void 0 ? void 0 : memory.hypocenter.depth) == 10 && (memory === null || memory === void 0 ? void 0 : memory.hypocenter.magnitude) == 0 && memory.maxInt == 5 && data.Hypocenter.includes('地方') : isLevel;
        const isPLUM2 = delay ? (memory === null || memory === void 0 ? void 0 : memory.hypocenter.depth) == 10 && (memory === null || memory === void 0 ? void 0 : memory.hypocenter.magnitude) == 10 && memory.maxInt != -1 : isPLUM;
    ...
    const callmaxint = (delay = false) => {
        const memory = EEWMemory.get(data.EventID);
        if (delay ? memory === null || memory === void 0 ? void 0 : memory.maxintcalled : maxintcalled)
            return;
        const isOnepoint2 = delay ? memory === null || memory === void 0 ? void 0 : memory.isOnepoint : isOnepoint;
        const isDeep2 = delay ? (memory === null || memory === void 0 ? void 0 : memory.maxInt) == -1 && memory.hypocenter.depth >= 150 : isDeep;
        const maxint = delay ? ((memory === null || memory === void 0 ? void 0 : memory.maxInt) || 0) : maxInt;
    ...
    for (const region of data.WarnArea) {
        regionmap[region.Chiiki] = Math.max(returnIntLevel(region.Shindo1), ((_a = regionmap[region.Chiiki]) !== null && _a !== void 0 ? _a : 0));
    }
    sendData({
        type: 'eewinfo',
        Delete: false,
