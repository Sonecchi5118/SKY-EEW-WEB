...
function clicktypeicon(type: number) {
  DisplayType = type
  changeDisplayType()
}
...
if (quakeicon) quakeicon.onclick = () => {clicktypeicon(0)};
if (realtimeicon) realtimeicon.onclick = () => {clicktypeicon(1)};
if (tsunamiicon) tsunamiicon.onclick = () => {clicktypeicon(2)};
if (settingicon) settingicon.onclick = () => {clicktypeicon(3)};
...
// RGBを補完する関数
function interpolateColor(value: number, breakpoints: {value: number, RGB: {r: number, g: number, b: number}}[]) {
...
const realtimequakeinfo: {maxInt: number; regions: {name: string; int: number;}[]} = {
  maxInt: -100,
  regions: []
}
